...
class eight_ball(commands.Cog):
    ...
    async def eight_ball(self, ctx):
        ...
        await ctx.send(
            f"{ctx.author.mention}: Hey, ask me a yes or no question! :smile:")

        try:
            question = await self.client.wait_for("message",
                                                  check=check,
                                                  timeout=10.0)

        except asyncio.TimeoutError:
            return await ctx.send(
                f"{ctx.author.mention}: Fine.. don't ask. :unamused:")
        ...
        if random_response[len(random_response) - 1] == "\n":
            return await ctx.send(
                f"{ctx.author.mention}: \"{question.content}\" - {random_response[:-1]}"
            )

        await ctx.send(
            f"{ctx.author.mention}: \"{question.content}\" - {random_response}"
        )
